...
// Conservative browser render budgets. Over-budget rows are reported, not fatal.
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (value === '--url') args.url = argv[++i];
    ...
    else if (value === '--view') args.view = argv[++i];
      ...
      console.log(
        'Usage: inspect-threejs-canvas.mjs [--url URL] [--out DIR] [--mobile] [--wait MS] [--state NAME] [--seed N]\n' +
          '  --state/--view/--seed drive window.__GNM_AVATAR_TEST_HOOKS__ before capture\n' +
          '  so specific avatar states can be measured deterministically.',
      );
      process.exit(0);
    } else {
...
// Objective pixel statistics computed on a coarse luminance grid so the
// inspection remains inexpensive.
...
function checkRenderBudget(renderer, mode) {
  ...
  return {
    tier: mode,
    note: 'conservative starting-point budget; document intentional overrides',
    rows,
...
async function sampleCanvas(page, mode, screenshotPath) {
  ...
  const dataUrl = await page.evaluate(
    () => window.__GNM_AVATAR_TEST_HOOKS__?.captureCanvasPng?.() ?? null,
  );
  const buffer = typeof dataUrl === 'string' && dataUrl.startsWith('data:image/png;base64,')
  ...
  const diagnostics = await page.evaluate(() => {
    ...
    return {
      drawingBuffer: canvas
        ...
        : null,
      avatar: window.__GNM_AVATAR_DIAGNOSTICS__ ?? null,
    };
  });
  ...
  return {
    ...
    renderBudget: checkRenderBudget(diagnostics.avatar?.renderer ?? null, mode),
    diagnostics: diagnostics.avatar,
  };
}
...
async function main() {
  ...
  if (args.state || args.view || args.seed !== undefined) {
    const applied = await page.evaluate(({ seed, state, view }) => {
      const hooks = window.__GNM_AVATAR_TEST_HOOKS__;
      if (!hooks) return false;
      if (typeof seed === 'number') hooks.seed?.(seed);
      if (state) hooks.setState?.(state);
      if (view) hooks.setView?.(view);
      return true;
    }, { seed: args.seed, state: args.state, view: args.view });
    if (!applied) {
      console.error(
        'warning: --state/--seed requested but __GNM_AVATAR_TEST_HOOKS__ is not defined; capturing the current state instead',
      );
    }
  }
  ...
  const baseName = [mode, args.view, args.state].filter(Boolean).join('-');
  const screenshotPath = path.join(args.out, `${baseName}.png`);
  const canvasScreenshotPath = path.join(args.out, `${baseName}-canvas.png`);
  const result = await sampleCanvas(page, mode, canvasScreenshotPath);
